...
// When compiling with nvcc, provide both device constant memory (for GPU) and host array (for CPU)
#ifdef __CUDACC__
// Device constant memory for GPU kernels
__device__ __constant__ WORD k_device[64] = {
...
// Make callable from both host and device code
__host__ __device__ void sha256_transform(SHA256 *ctx, const BYTE *msg)
{
	WORD a, b, c, d, e, f, g, h;
	WORD i, j;

	// Select correct k array: device constant memory on GPU, host array on CPU
	#ifdef __CUDA_ARCH__
	...
	// Sliding window optimization: Use circular buffer w[16] instead of w[64]
	// This reduces register pressure from 64 words (256 bytes) to 16 words (64 bytes)
	// Compute w[i] on-the-fly just before use in compression loop
	WORD w[16];
	...
	// Merged message schedule + compression loop
	// Compute w[i] just before using it in compression
	// Unroll to reduce loop overhead and improve instruction-level parallelism
	#pragma unroll
	for(i=0;i<64;++i)
	{
		WORD w_i;
		...
		if (i < 16)
		{
			// First 16: load from message
			j = i * 4;
			w_i = (msg[j]<<24) | (msg[j+1]<<16) | (msg[j+2]<<8) | (msg[j+3]);
		}
		else
		{
			// Rest: compute from circular buffer
			// Access w[i-2], w[i-7], w[i-15], w[i-16] using modulo 16 indexing
			WORD w_2  = w[(i - 2)  & 15];
			WORD w_7  = w[(i - 7)  & 15];
			WORD w_15 = w[(i - 15) & 15];
			WORD w_16 = w[(i - 16) & 15];  // Same slot as w[i], will be overwritten

			WORD s0 = (_rotr(w_15, 7)) ^ (_rotr(w_15, 18)) ^ (w_15>>3);
			WORD s1 = (_rotr(w_2, 17)) ^ (_rotr(w_2, 19))  ^ (w_2>>10);
			w_i = w_16 + s0 + w_7 + s1;
		}

		// Store in circular buffer (will be needed for future iterations)
		w[i & 15] = w_i;

		// Compression (use w_i immediately)
		WORD S0 = (_rotr(a, 2)) ^ (_rotr(a, 13)) ^ (_rotr(a, 22));
		WORD S1 = (_rotr(e, 6)) ^ (_rotr(e, 11)) ^ (_rotr(e, 25));
		WORD ch = (e & f) ^ ((~e) & g);
		WORD maj = (a & b) ^ (a & c) ^ (b & c);
		WORD temp1 = h + S1 + ch + k[i] + w_i;  // Use w_i directly
		WORD temp2 = S0 + maj;
...
// Make callable from both host (CPU) and device (GPU) code
__host__ __device__ void sha256(SHA256 *ctx, const BYTE *msg, size_t len)
{
	...
	if(j > 56)
	{
		...
		memset(m, 0, sizeof(m));
		// printf("true\n");  // Disabled: printf not allowed in device code
	}
...
// Process exactly one 64-byte chunk and update the SHA-256 state
// This is used to compute the midstate after processing the first 64 bytes
__host__ __device__ void sha256_update_64(SHA256 *ctx, const BYTE *msg)
{
	sha256_transform(ctx, msg);
}

// Finalize SHA-256 hash for an 80-byte message given the last 16 bytes
// Assumes ctx contains the midstate after processing first 64 bytes
// This adds proper padding for an 80-byte message and produces final hash
__host__ __device__ void sha256_finalize_80(SHA256 *ctx, const BYTE *last_16_bytes)
{
	// Build final 64-byte chunk:
	// - First 16 bytes: remaining data (bytes 64-79 of original message)
	// - Byte 16: 0x80 (padding bit)
	// - Bytes 17-61: zeros
	// - Bytes 62-63: message length in bits (80 * 8 = 640 = 0x0280 big-endian)
	BYTE final_chunk[64];

	// Copy last 16 bytes of data
	for(int i = 0; i < 16; i++)
	{
		final_chunk[i] = last_16_bytes[i];
	}

	// Add padding
	final_chunk[16] = 0x80;  // Append '1' bit

	// Zero out bytes 17-61
	for(int i = 17; i < 62; i++)
	{
		final_chunk[i] = 0x00;
	}

	// Append length: 80 bytes * 8 = 640 bits = 0x0280 in big-endian
	final_chunk[62] = 0x02;
	final_chunk[63] = 0x80;

	// Process final chunk
	sha256_transform(ctx, final_chunk);
